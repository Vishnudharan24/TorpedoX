...
import torch
import torch.nn as nn
class FeedForwardDecisionFusion:
    def __init__(self, num_classes, paragraph_size, num_paragraphs):
        ...
        self.model = FeedForwardNN(input_size=500, num_classes=num_classes)  # Adjust input_size as needed
    ...
    def _preprocess_ciphertext(self, ciphertext):
        ...
        tensor_data = torch.tensor(numeric_data).float().unsqueeze(0)
        return tensor_data

    def _combine_analysis_data(self, linear_results, higher_order_results, analysis_results):
        # Handle cases where results are dictionaries
        if isinstance(linear_results, dict):
            linear_tensor = torch.tensor(list(linear_results.values())).float()
        else:
            linear_tensor = torch.tensor(linear_results).float()
        
        if isinstance(higher_order_results, dict):
            higher_order_tensor = torch.tensor(list(higher_order_results.values())).float()
        else:
            higher_order_tensor = torch.tensor(higher_order_results).float()
        
        if isinstance(analysis_results, dict):
            analysis_tensor = torch.tensor(list(analysis_results.values())).float()
        else:
            analysis_tensor = torch.tensor(analysis_results).float()
        
        # Concatenate tensors along dimension 1 (features)
        combined_data = torch.cat((linear_tensor, higher_order_tensor, analysis_tensor), dim=1)
            ...
            for paragraphs in train_loader:
                for paragraph in paragraphs:
                    ...
                    for ciphertext, algo_name in ciphertexts:
                        ...
                        # Ensure the results are tensors before combining
                        combined_data = self._combine_analysis_data(linear_results, higher_order_results, analysis_results)
                        
                        cnn_output = self.forward(input_tensor)
                        
                        # Concatenate CNN output with combined data
                        combined_data = torch.cat((cnn_output, combined_data), dim=1)
...
# Initialize the FeedForwardDecisionFusion model
num_classes = 9  # Number of different encryption algorithms
paragraph_size = 128  # Size of each paragraph
num_paragraphs = 10  # Number of paragraphs

model = FeedForwardDecisionFusion(
