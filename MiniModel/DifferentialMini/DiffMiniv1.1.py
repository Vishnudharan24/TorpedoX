    ...
    for _ in range(num_samples):
        for encrypt_func, label in crypto_utils.algorithms:
            ...
            if 'Statistical Analysis' in analysis_result:
                ...
                #                  mu, std, p_value, autocorr, heatmap_data,clustering_coefficients])
                # labels.append(label)
...
features, labels = generate_data(num_samples=10)
...
# X_train_cleaned = X_train[:, np.apply_along_axis(lambda col: np.all(np.char.isnumeric(col)), 0, X_train)]
